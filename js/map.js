...
function getDistance(p0, p1) {
  if (p0.getGeometry) p0 = p0.getGeometry().getCoordinates();
  if (p1.getGeometry) p1 = p1.getGeometry().getCoordinates();
  // console.log(p0, p1)
  ...
  baseUpdate(frameCount) {
    if (map.automove) {
      moveMarker({
        ...
        r: AUTOMOVE_SPEED,
        theta: 10 * noise.noise2D(frameCount * 0.01, 1),
        lerpTo: NU_CENTER,
        lerpAmt: 0.01,
      });
    }
      ...
      if (!landmark.isPlayer) {
        ...
        this.ranges.forEach((range, index) => {
          if (landmark.distanceToPlayer < range) {
            maxLevel = index;
            // right now: level 0 is 500m, 1 is 200m, 2 is 90m, 3 is 1m
